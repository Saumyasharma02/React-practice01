...
function App() {
  ...
  const passwordGenerator=useCallback(()=>{
    ...
    for(let i=1;i<length;i++)//for appending chars upto i length
    {
  ...
  const copy=useCallback(()=>{
    pref.current?.select()
    //pref.current?.setSelectionRange(0,9)
  ...
  return (
    <>
      <div className='w-full max-w-md mx-auto shadow-md rounded-lg px-8 my-8 bg-gray-800 text-orange-500'>
        <h1 className='text-2xl text-center my-3'>Password Generator</h1>
        <div className='flex shadow rounded-lg overflow-hidden mb-4'>
          <input type="text" value={password} className='outline-none w-full py-1 px-3' placeholder='password' readOnly ref={pref}/>
        ...
        <div className='flex text-sm gap-x-2 py-2'>
          ...
          <div className='flex items-center gap-x-1'>
            <input type="checkbox" defaultChecked={numAllowed} id='numberallowed' onChange={()=>{setNumAllowed((prev)=>!prev)}} />
            <label>Numbers</label>
          </div>
          <div className='flex items-center gap-x-1'>
